...
{
	char *label = PEAP_TLV_IPMK_SEED_LABEL;
...
static void verify_compound_mac(u_char *in_buf)
{
	...
	for (int i = 0; i < PEAP_TLV_CMK_LEN; i++)
		if (in_buf[PEAP_TLV_NONCE_LEN + i] != out_buf[PEAP_TLV_HEADERLEN + PEAP_TLV_NONCE_LEN + i])
			fatal("server's CMK does not match client's CMK, potential MiTM");
}

static void generate_mppe_keys(void)
{
	char *label = PEAP_TLV_CSK_SEED_LABEL;
...
static void peap_init(int unit)
{
	SSL_library_init();
	SSL_load_error_strings();
}

struct protent peap_protent = {
	PPP_PEAP,
	peap_init,
	NULL,
	NULL,
